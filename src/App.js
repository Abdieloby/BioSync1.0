...
const AICoachView = ({ history, saveEntry }) => {
  ...
  const callGemini = async (queryText, base64Image) => {
    ...
    try {
      ...
      // ERROR HANDLING: Return the specific error message if AI fails
      if (data.error) {
      ...
      return `Connection failed: ${err.message}. This is usually caused by an ad-blocker or poor connection.`;
    }
  };
  ...
  const handleSend = async () => {
    ...
    if (res && res.includes("ACTION_SAVE:")) {
      try {
    ...
    setMessages((prev) => [
      ...
      {
        role: "assistant",
        content: (res || "Error: No response text found.")
          .replace(/ACTION_SAVE:.*(\n|$)/g, "")
          .trim(),
      },
    ]);
